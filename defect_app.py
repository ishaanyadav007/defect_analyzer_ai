import streamlit as st  # in order to import the funciton to design the interface
from dotenv import load_dotenv
load_dotenv() # to access API key frmo .env file

import google.generativeai as genai # to access the models

from PIL import Image # pillow is used to load,save, convert the format and manipulate te image

## STREAMLIT PAGE

st.set_page_config(page_title='Structural Defect analysis using AI',
                   page_icon='🏚️', layout='wide' )
st.title('AI assistant for :green[Structural Defect Anlaysis] 🏚️🏢')
st.header(':blue[Prototype for automated structural defect analysis ]')
st.subheader('Develop a web based app using Streamlit that allows users to upload \
             images of building structures and to analyse the defects using Gemini model.',
             divider=True)

with st.expander('About the application'):
    st.markdown(f'''This is used to detect the strutural defect in the given images
                like cracks, misallignments using AI models
                - ***Defect Detection***
                -***Recommendations***
                -***Report Generation***''')

input_image = st.file_uploader('Click Here',type=['jpeg','webp','jpg','png'])

img=''

if input_image:
    img = Image.open(input_image).convert('RGB')
    st.image(img,caption='Uploaded usccessfully.')
...
def generate_results(prompt,img):
    ...
    results = model.generate_content(f''' Using the image{img}, answer the prompt{prompt}.''')
    return results.text
...
if submit:
    with st.spinner('Analyzing.......'):
        ...
        st.markdown('## The response of genai is:')
        st.write(response)